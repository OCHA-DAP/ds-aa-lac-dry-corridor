...
tar_load(df_all_thresholds_rp4)
...
gt_primera <- df_primera_thresholds %>% 
  ...
  tab_header(title = "Primera (MJJA 2024) Rainfall (mm) Monitoring Thresholds",
             subtitle = gt_legend,
             preheader = "sdfasdfasdfasdfadfadsfas")
...
## Postrera
```{r}
ecmwf_postrera_thresholds <- ldf_ecmwf$postera %>% 
  mutate(
    pub_month_int = 9- lt,
    pub_month_chr = month(pub_month_int, label =T, abb=T),
    pub_month_lt = paste0(pub_month_chr," (", lt,")")
...
insuv_postrera_thresholds <- ldf_insuv$postera %>% 
  mutate(
        pub_month_int = 9- lt,
        pub_month_chr = month(pub_month_int, label =T, abb=T),
        pub_month_lt = paste0(pub_month_chr," (", lt,")")
