...
**NOTE:** To create this file (`02b_joint_activation_rates_cds.qmd` ) I just copied `02a_joint_activation_rates_mars.qmd` and changed the inputs. `02a` has already been reviewed. Given the time constraints associated with the shift in data source for the trigger, this was simply the fastest way to re-create the table for the updated technical note:
...
tar_load(df_cds_seasonal_summarised, store = here("_targets"))
tar_load(df_cds_insivumeh_thresholds_rp4,store = here("_targets"))

ldf_thresholds <- df_cds_insivumeh_thresholds_rp4 %>% 
  split(.$forecast_source)
  
df_ecmwf_classified <- df_cds_seasonal_summarised %>% 
  filter(
  ...
  left_join(ldf_thresholds$`ECMWF CDs`) %>% 
...
df_joint_by_season_ecmwf <- df_ecmwf_classified %>% 
 ...
 df_insuv_classified<- df_insuvimeh_seasonal_summarised_filtered %>% 
  ...
  left_join(ldf_thresholds$INSUVIMEH) %>% 
...
Joint activation across all lead times per season for just INSUVIMEH data
