...
# Option A per RP: best avg_f1 (within tolerance), tiebroken by lowest CV
hnd_opt_a <- hnd_all_hl |>
  group_by(annual_rp) |>
  filter(avg_f1 >= max(avg_f1) - 0.01) |>
  arrange(all_cv) |>
  slice_head(n = 1) |>
  ungroup() |>
  mutate(option = "A", selection = "max avg F1")

# Option B per RP: best era5_tol_f1 (within tolerance), tiebroken by lowest CV
hnd_opt_b <- hnd_all_hl |>
  group_by(annual_rp) |>
  filter(era5_tol_f1 >= max(era5_tol_f1) - 0.01) |>
  arrange(all_cv) |>
  slice_head(n = 1) |>
  ungroup() |>
