...
The plot below shows each candidate configuration positioned by its ERA5 tolerant F1 (x-axis) and impact F1 (y-axis). Configurations near the top-right corner perform well on both metrics. Yellow circles highlight configurations where a single RP threshold is used uniformly across all leadtimes and seasons --- an operationally favourable property that simplifies trigger communication and implementation. Hover over any point for details; click circled points to view the full per-leadtime activation table.
...
# Flag configs with all equal thresholds (uniform across LTs and seasons)
hnd_uniform <- hnd_df |>
  filter(apply(hnd_df[, threshold_cols_hnd], 1, function(x) length(unique(x)) == 1))
...
p_hnd <- ggplot(mapping = aes(x = era5_tol_f1, y = impact_f1)) +
  geom_point_interactive(
    ...
    aes(tooltip = sprintf("RP %.2f | avg F1 = %.3f | CV = %.3f\nImpact = %.3f | ERA5 tol = %.3f",
                          annual_rp, avg_f1, mean_cv, impact_f1, era5_tol_f1)),
  ...
  {if (nrow(hnd_uniform) > 0) geom_point(
    data = hnd_uniform, color = "#ffd700", size = 9,
    shape = 16, alpha = 0.5, show.legend = FALSE
  )} +
  geom_point_interactive(
    data = hnd_final,
    aes(color = rp_group,
  ...
  labs(x = "ERA5 Tolerant F1", y = "Impact F1 (EM-DAT)",
       ...
       subtitle = "Click circled points for detail | Yellow = uniform thresholds across all LTs") +
...
# Flag configs with all equal thresholds
slv_uniform <- slv_df |>
  filter(apply(slv_df[, threshold_cols_slv], 1, function(x) length(unique(x)) == 1))
...
p_slv <- ggplot(mapping = aes(x = era5_tol_f1, y = impact_f1)) +
  ...
  {if (nrow(slv_uniform) > 0) geom_point(
    data = slv_uniform, color = "#ffd700", size = 9,
    shape = 16, alpha = 0.5, show.legend = FALSE
  )} +
  geom_point_interactive(
    data = slv_final,
...
target_rp_gtm <- (EVAL_END - EVAL_START + 1 + 1) / length(impact_yrs_gtm)
```
...
gtm_final <- bind_rows(gtm_opt_a, gtm_opt_b) |>
...
# Flag configs with all equal thresholds
gtm_uniform <- gtm_df |>
  filter(apply(gtm_df[, threshold_cols_gtm], 1, function(x) length(unique(x)) == 1))
...
p_gtm <- ggplot(mapping = aes(x = era5_tol_f1, y = impact_f1)) +
  ...
  {if (nrow(gtm_uniform) > 0) geom_point(
    data = gtm_uniform, color = "#ffd700", size = 9,
    shape = 16, alpha = 0.5, show.legend = FALSE
  )} +
  geom_point_interactive(
    data = gtm_final,
...
target_rp_gtmsn <- (EVAL_END - EVAL_START + 1 + 1) / length(impact_yrs_gtmsn)
```
...
trigger_data_gtmsn <- build_trigger_lookup(df_gtm_sn, min_rp = 2.8, max_rp = 15,
                                            primera_lts = primera_lts_gtmsn,
                                            postrera_lts = postrera_lts_gtmsn)
...
gtmsn_final <- bind_rows(gtmsn_opt_a, gtmsn_opt_b) |>
...
# Flag configs with all equal thresholds
gtmsn_uniform <- gtmsn_df |>
  filter(apply(gtmsn_df[, threshold_cols_gtmsn], 1, function(x) length(unique(x)) == 1))
...
p_gtmsn <- ggplot(mapping = aes(x = era5_tol_f1, y = impact_f1)) +
  ...
  {if (nrow(gtmsn_uniform) > 0) geom_point(
    data = gtmsn_uniform, color = "#ffd700", size = 9,
    shape = 16, alpha = 0.5, show.legend = FALSE
  )} +
  geom_point_interactive(
    data = gtmsn_final,
