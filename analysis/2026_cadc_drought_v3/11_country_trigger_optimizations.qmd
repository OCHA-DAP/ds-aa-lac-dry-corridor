...
# no need to separate SN into the different monitoring AOIS for this overview plot
...
# note: alot of complicated code, but really just to get the yellow hallows on
# the plot
...
# Join by threshold cols (scoped to active LTs only — no NAs in join keys)
...
# this static version is better for framework doc whereas interactive is better for online book.
...
# Selected config: RP 2.9 A
hnd_fw_best <- hnd_final |> filter(config_label == "RP 2.9 A")
...
ggplot(mapping = aes(x = era5_tol_f1, y = impact_f1)) +
  ...
  geom_label_repel(
    data = hnd_fw_best,
    aes(label = sprintf("RP %.1f\navg F1=%.2f", annual_rp, avg_f1), color = rp_group),
  ...
  labs(x = "ERA5 Tolerant F1", y = "Impact F1 (EM-DAT)",
       ...
       subtitle = "Circled = selected configuration (RP 2.9 A)") +
...
# Selected config: RP 2.9 A
slv_fw_best <- slv_final |> filter(config_label == "RP 2.9 A")
...
# Selected config: RP 2.9 A
gtm_fw_best <- gtm_final |> filter(config_label == "RP 2.9 A")
...
# note to reviewers: after running this chunk, you can skip down
# to GTMSN Primera Only section (~ln 3433) which is where the final config for SN
# is made.... but it does reqire the hybrid GT15 (p2 only) and merged admin
# trigger_data objects created in this chunk
...
# here we replace trigger lookup for p2 w/ that from GT15 only
...
# here we replace rp_options for p2 w/ that from GT15 only
...
# realize that we are for p2 we evaluating GT15-only trigger/forecast
# against the merged ERA rainfall for the two admins -- this actually
# makes sense and is okay considering if this triggers activities can 
# occur in either (how well does forecat in A generaliaze to rainfall
# defecits in A + B)
...
# Selected config: RP 4.4 Uniform (thresh 5.0)
gtmsnp_fw_best <- gtmsnp_final |>
...
ggplot(mapping = aes(x = era5_tol_f1, y = impact_f1)) +
  ...
  labs(x = "ERA5 Tolerant F1", y = "Impact F1 (EM-DAT)",
       ...
       subtitle = "Circled = selected configuration (RP 4.4 Uniform)") +
  theme_minimal(base_size = 11) +
  theme(legend.position = "right", panel.grid.minor = element_blank())
```
...
# ── RP → mm conversion ────────────────────────────────────────────────────
# Drought-only: rank(x) gives rank 1 to the smallest (driest) value,
# so lowest rainfall → highest RP. Equivalent to calc_rp_threshold(x, direction=-1)
# in R/country_trigger_optimization.R — no direction param needed here.
