...
lt_list <- list(
  ...
  list(window = "postrera", leadtime = 2, id = "s2")
)
...
compute_matched <- function(row) {
  ...
  s2_yrs <- if (!is.na(row$s2)) trigger_lookup[[sprintf("s2_%.4f", row$s2)]] else integer(0)

  # Season-level trigger years (OR-union)
  p_trigger_yrs <- Reduce(union, list(p0_yrs, p1_yrs, p2_yrs))
  s_trigger_yrs <- Reduce(union, list(s0_yrs, s1_yrs, s2_yrs))
  ...
  s_tp_lt2 <- length(intersect(s2_yrs, s_drought))
  s_tp_lt1_only <- length(setdiff(intersect(s1_yrs, s_drought), s2_yrs))
  s_tp_lt0_only <- length(setdiff(intersect(s0_yrs, s_drought), union(s1_yrs, s2_yrs)))
  s_fp_lt2_marg <- length(setdiff(setdiff(s2_yrs, s_drought), union(s0_yrs, s1_yrs)))
  ...
  tibble(
    ...
    s_tp_earliest_lt2 = s_tp_lt2, s_tp_earliest_lt1 = s_tp_lt1_only,
    s_tp_earliest_lt0 = s_tp_lt0_only,
    s_fp_marginal_lt2 = s_fp_lt2_marg, s_fp_marginal_lt1 = s_fp_lt1_marg
  )
}
...
configs <- configs_314 |>
  ...
  mutate(
    ...
    s_early_tp = s_tp_earliest_lt2 + s_tp_earliest_lt1,
    total_early_tp = p_early_tp + s_early_tp,
    p_marginal_fp = p_fp_marginal_lt2 + p_fp_marginal_lt1,
    s_marginal_fp = s_fp_marginal_lt2 + s_fp_marginal_lt1,
    ...
    s_cv = pmap_dbl(list(s0, s1, s2), function(...) {
      vals <- c(...) |> na.omit()
      if (length(vals) < 2) return(0)
      sd(vals) / mean(vals)
    }),
...
We apply a sequential filtering pipeline to narrow from `r format(nrow(configs), big.mark = ",")` configs to the top candidates:

```{r}
#| label: compute-filtering

# Helper to format top configs for display
fmt_top <- function(df, n = 5) {
  df |>
    arrange(desc(mean_f1), mean_cv) |>
    head(n) |>
    mutate(across(c(mean_f1, primera_f1, postrera_f1), \(x) sprintf("%.2f", x)),
           across(c(mean_cv, rp_diff), \(x) sprintf("%.3f", x)),
           across(c(p_seasonal_rp, s_seasonal_rp), \(x) sprintf("%.1f", x)))
}
...
# Step 3: Rank by mean CV (lower = more uniform thresholds)
candidates <- step2 |> arrange(mean_cv)
```

```{r}
#| label: fig-filter-pipeline
#| fig-cap: "Filtering pipeline from full search space to candidate configurations. Each step applies one criterion."
#| fig-height: 4
#| fig-width: 9
...
pipeline <- tibble(
  step = c(
    ...
    sprintf("Ranked by mean CV\n(%s candidates)", nrow(candidates))
  ),
  n = c(nrow(configs), nrow(step1), nrow(step2), nrow(candidates))
) |>
...
#| label: tbl-step1-top
fmt_top(step1) |>
  select(p0, p1, p2, s0, s1, s2, mean_f1, primera_f1, postrera_f1, rp_diff, mean_cv) |>
  gt() |> cols_align(align = "center") |>
  tab_header(title = sprintf("Top 5 of %s configs (by mean F1)", format(nrow(step1), big.mark = ",")))
```
...
#| label: tbl-step2-top
fmt_top(step2) |>
  select(p0, p1, p2, s0, s1, s2, mean_f1, mean_cv, rp_diff, p_seasonal_rp, s_seasonal_rp) |>
  gt() |> cols_align(align = "center") |>
  tab_header(title = sprintf("Top 5 of %s configs (by lowest CV)", format(nrow(step2), big.mark = ",")))
```

### Ranking: Threshold Uniformity (Mean CV)

Among the `r nrow(candidates)` tied-F1 configs, we rank by mean coefficient of variation of thresholds within each season. Lower CV means the thresholds are more similar across leadtimes, which is operationally simpler. All candidates share the same primera thresholds and postrera LT0/LT1 — they differ only in the **postrera LT2 (July)** threshold.
...
candidates |>
  select(p0, p1, p2, s0, s1, s2, mean_f1, primera_f1, postrera_f1,
         p_seasonal_rp, s_seasonal_rp, annual_rp, mean_cv,
         total_early_tp, total_marginal_fp) |>
  mutate(across(c(mean_f1, primera_f1, postrera_f1, mean_cv), \(x) round(x, 3)),
         across(c(p_seasonal_rp, s_seasonal_rp, annual_rp), \(x) round(x, 2)),
         s2 = if_else(is.na(s2), "—", sprintf("%.1f", s2))) |>
  gt() |>
  cols_align(align = "center") |>
  tab_header(
    title = sprintf("Candidate configurations (ranked by mean CV)"),
    subtitle = sprintf("All share mean F1 = %.2f | Sorted by threshold uniformity", max_f1)
  ) |>
  tab_footnote(
    footnote = sprintf("Annual RP = %.2f (trigger in %d of %d years)",
                        candidates$annual_rp[1], round(N_YEARS / candidates$annual_rp[1]), N_YEARS)
  )
```
...
candidates |>
  ...
  gt() |>
  cols_align(align = "center") |>
  tab_header(title = "Best candidate with postrera LT2 (July) included")
```
...
s2_yrs <- if (!is.na(winner$s2)) trigger_lookup[[sprintf("s2_%.4f", winner$s2)]] else integer(0)

p_union <- Reduce(union, list(p0_yrs, p1_yrs, p2_yrs))
s_union <- Reduce(union, list(s0_yrs, s1_yrs, s2_yrs))
...
lt_data <- bind_rows(
  ...
  classify_lt(s2_yrs, integer(0),            s_drought, "LT2 (Jul)", "Postrera"),
  classify_lt(s1_yrs, s2_yrs,               s_drought, "LT1 (Aug)", "Postrera"),
  classify_lt(s0_yrs, union(s1_yrs, s2_yrs), s_drought, "LT0 (Sep)", "Postrera")
)
...
fcst_rp <- df_joined |>
  ...
  mutate(
    row = case_when(
      ...
      window == "postrera" & leadtime == 2 ~ "LT2 (Jul)",
...
p_postrera <- build_season_plot(
  "Postrera",
  c("LT2 (Jul)", "LT1 (Aug)", "LT0 (Sep)", "Season trigger", "Observed (ERA5)"),
  show_x = TRUE
)
...
ew_data <- tibble(
  Leadtime = c("LT2 (Mar)", "LT1 (Apr)", "LT0 (May)", "LT2 (Jul)", "LT1 (Aug)", "LT0 (Sep)"),
  Season = c(rep("Primera", 3), rep("Postrera", 3)),
  `TPs first caught` = c(winner$p_tp_earliest_lt2, winner$p_tp_earliest_lt1,
                          winner$p_tp_earliest_lt0,
                          winner$s_tp_earliest_lt2, winner$s_tp_earliest_lt1,
                          winner$s_tp_earliest_lt0),
  `Marginal FPs` = c(winner$p_fp_marginal_lt2, winner$p_fp_marginal_lt1, 0,
                      winner$s_fp_marginal_lt2, winner$s_fp_marginal_lt1, 0)
) |>
...
The Honduras-specific trigger optimization identifies `r nrow(candidates)` candidate configurations:

- **Primera**: LT0 = RP `r sprintf("%.1f", winner$p0)`, LT1 = RP `r sprintf("%.0f", winner$p1)`, LT2 = RP `r sprintf("%.0f", winner$p2)`
- **Postrera**: LT0 = RP `r sprintf("%.1f", winner$s0)`, LT1 = RP `r sprintf("%.1f", winner$s1)``r if (!is.na(winner$s2)) sprintf(", LT2 = RP %.1f", winner$s2) else ""`
...
5. **Ranked by threshold uniformity** (mean CV) → `r nrow(candidates)` candidates
