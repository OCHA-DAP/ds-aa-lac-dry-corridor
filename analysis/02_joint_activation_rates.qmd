...
df_ecmwf_classified <- df_mars_seasonal_summarised %>% 
  filter(
    # not monitorng in Feb or Sep
    !(month(pub_date) %in% c(2,9))
  ) %>% 
...
df_joint_by_season_ecmwf <- df_ecmwf_classified %>% 
 ...
 df_insuv_classified<- df_insuvimeh_seasonal_summarised_filtered %>% 
   # not monitoring Feb, sep already filtered out as insuvimeh only gives pub_mo + 1
   # not pub_mo = valid_mo
  filter(month(pub_date)!=2) %>% 
...
df_joint_activations_combined_wide  %>% 
  mutate(
    window = fct_relevel(window, "primera","postera","combined")
  ) %>% 
  arrange(window) %>% 
  gt() %>% 
