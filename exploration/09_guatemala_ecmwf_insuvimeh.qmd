...
tar_load(df_gtm_nextgen_adm0,store = here("_targets"))
tar_load(df_ecmwf_mars,store = here("_targets"))

df_mars_insuvimeh <- bind_rows(
 df_ecmwf_mars %>% 
...
df_mars_insuvimeh$pub_date %>% range()
df_mars_insuvimeh %>% 
  filter(year(pub_date) %in% c(1983:2022)) %>% 
  mutate(
    decade =year(pub_date) - year(pub_date) %% 10,
    decade_abbr = paste0(decade,"'s")
  ) %>% 
  ggplot(
    aes(x= valid_date,y= value, color= source)
  )+
...
df_mars_insuvimeh %>% 
  filter(year(pub_date) %in% c(1983:2022)) %>%  
  select(pub_date,valid_date, lt,source,mm=value) %>% 
  pivot_wider(names_from=source, values_from=mm) %>% 
  filter(!is.na(`ecmwf mars`),!is.na(insuvimeh)) %>% 
  ggplot(
    aes(x= `ecmwf mars`,y= insuvimeh)
  )+
...
debugonce(ecmwf_summarise_seasons)
# this is the above for insuvimeh
insuvimeh_seasons <- ecmwf_summarise_seasons(df = df_mars_insuvimeh %>% 
                                               rename(mm=value) %>% 
                          filter(source=="insuvimeh") %>% 
                            mutate(lt = lt+1),
