...
2. ≥ 25 % of cropland with VHI ≤ 0.25 - only monitored for Primera (phase 2) and Postera (phase 3)
...
df_vhi_season <- df_cropland_lte_vhi_threshold %>% 
  mutate(
    ...
    monitoring_window = case_when(
      # Primera 2 is JJA- there fore monitoring MJJ
 ...
 # i might have to play around with these parameters if I want to knit or export pngs
...
df_iri_adm0_flag %>% 
  ...
  labs(
    title = "Dry Corridor Central America",
    subtitle= "Green Cells Indicate Where IRI Seasonal Forecast Thresholds were met\nand Black outline indicates when VHI threshold was met"
  )+
  scale_x_continuous(
    breaks= unique(df_iri_adm0_flag$date_cat),
    labels= unique(df_iri_adm0_flag$seas)
