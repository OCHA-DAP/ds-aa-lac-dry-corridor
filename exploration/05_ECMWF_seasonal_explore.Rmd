...
How about converting to trimester
```{r}
library(zoo)
df_regional_trim %>% 
  print(n=100)
df_regional_trim <- df_zonal_regional %>% 
  rename(
    date_predict = "date"
  ) %>% 
  mutate(
  ...
  # group_by(date_forecast_gen) %>%
  # mutate(
  #   roll3= rollsumr(total_mean_mm, 3, fill = NA)
  # )
    mutate(
...
#   ) %>% 
#   summarise(
#     total_mm= sum(total_mean_mm,na.rm=T)
#   )
# 
# df_regional_trim %>% 
#   print(n=100)
...
df_regional_trim_sum <- df_regional_trim %>% 
    group_by(
    yr = floor_date(date_predict,"year"),
    date_forecast_gen,
    window
...
df_regional_trim_max_per_year <- df_regional_trim_sum %>% 
  group_by(
    yr, window
  ) %>% 
...
ck$`lgl_jja` %>% 
  ...
  summarise(
    mean_mm =sum(total_mean_mm, na.rm = TRUE),
    n=n()
  )
  ...
  mutate(
    ...
    roll3 = rollsumr(total_mean_mm, 3, fill = NA, align = "right"),
    start_mo_tri = month(date)
  ) %>% 
  group_by(date,start_mo_tri) %>%
  summarise(
    # total_mean_mm = mean(total_mean_mm, na.rm = TRUE),
    roll3 = max(roll3, na.rm = TRUE),.group="drop"
  ) %>% 
