...
#. define useful funcs here
available_leadtime_months <-  function(x){
    valid_months <- as.numeric(x)
    least_recent <- max(valid_months)
    most_recent <- min(valid_months)
    furthest_back<- least_recent -5
    monitor_months <- furthest_back:most_recent
    return(monitor_months)
}

adjustable_leadtimes <-  function(monitor_months, forecast_months){
  mon_mo <-  as.numeric(monitor_months)
  forecast_start <- as.numeric( min(forecast_months))
...
# inputPanel(
  ...
  renderUI({
    monitor_months <- available_leadtime_months(input$winA_forecast)
    checkboxGroupInput(
      inputId= "winA_mon",
...
conditionalPanel(condition = "input.num_windows >= 2",
                 inputPanel(
                   ...
                   renderUI({
                     monitor_months <- available_leadtime_months(input$winB_forecast)
                     checkboxGroupInput(
                       inputId= "winB_mon",
...
```{r echo=F,eval=F}
input.winA_mon <-  c(3,4,5)
input.winA_mon <-  c(6,7,8,9)
input.winA_forecast <-  c(5,6,7,8)
input.winA_forecast <-  c(9,10,11)
input.winaA_mon <- available_leadtime_months(x = input.winA_forecast)

 df_ecmwf_win_filt <- df_ecmwf_zonal_all %>% 
  ...
  df_ecmwf_win_filt <- df_ecmwf_win_filt %>% 
    group_by(adm0_es, pub_date) %>% 
    filter(
      all(input.winA_forecast %in% valid_mo)
    )
  
  df_win1 <- df_ecmwf_win_filt%>% 
    summarise(
      mm = sum(mm),
      lt= min(lt)
...
df_win1_int <- df_ecmwf_zonal_all %>% 
  mutate(
    lt= lt- 1
  ) %>% 
  filter(stat== "mean") %>% 
...
df_ecmwf_zonal_filt_mins <- df_ecmwf_zonal %>% 
      ...
      # (seas_gen == "MJJ" & month(date) %in% input$w1_monitor)|
        ...
        (seas_gen == "SON" & month(date) %in% input.winA_mon)
         )
    ) %>%
  group_by(
...
# df_era_wv2_wide<- df_era5_mo %>% 
#   group_by(yr_date=floor_date(date, "year"), f_v2_win) %>% 
#   summarise(
#     mm= sum(value)*1000
#   ) %>% 
#   filter(!is.na(f_v2_win)) %>% 
#   pivot_wider(
#     names_from = f_v2_win,
#     values_from = mm
#   ) 
...
# df_seas51_wv2 %>% 
#   left_join(df_era_wv2_wide) %>% 
#   ggplot(
#     aes(x= mm,y= win_A)
#   )+
#   geom_point()+
#   facet_grid(rows= vars(lt), cols = vars(adm0_es))
...
# l_rps = c(`0` = 4,`1` =3,`2`=3)
# l_rps = c(`1` = 3,`2` =4,`3`=4,`4`=4)
dynamic_rps <- reactive({
  l_rps <- gather_slider_values()
  df_win_wide <- df_win1() %>% 
          select(-pub_date) %>% 
          pivot_wider(names_from = lt,values_from = mm)
  
  cat("printing df_win_wide in dynamic_rps()\n")
  print(df_win_wide)
  ...
  ldf_rps <- l_rps %>% 
    imap(
      \(rp,nm){
        ...
        df_ecmwf_thresh <- df_win_wide  %>% 
          # select(-pub_date) %>% 
          # pivot_wider(names_from = lt,values_from = mm) %>% 
          group_by(adm0_es) %>%
          reframe(
            # !!sym(as.character(nm)) := quantile(!!sym(as.character(nm)),q_rp),
            thresh = quantile(!!sym(as.character(nm)),q_rp),
            .groups="drop"
          ) %>% 
          mutate(
            lt = nm
  ...
  # shifted from purrr::reduce(left_join) as i was gettting error about .init & .x
  ldf_rps %>% 
    list_rbind() %>% 
    pivot_wider(
      names_from =lt,
    ...
    # reduce(.x= ldf_rps,.f=left_join,by="adm0_es")
  })
...
df_ecmwf_vhi_jja <- reactive({
  ...
  df_vhi_pct_avg <- df_vhi_plot() %>% 
    ...
    summarise(
      pct = mean(pct_crop_pixels_lte_thresh),.groups="drop"
    )
  df_ecmwf_jja %>% 
