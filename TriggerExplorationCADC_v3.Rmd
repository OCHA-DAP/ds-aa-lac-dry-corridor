...
adjustable_leadtimes2_reactive_b <- reactive({
  # browser()
  adjustable_leadtimes2(publication_mo = as.numeric(input$winB_mon),
                        valid_mo = as.numeric(input$winB_forecast))
})
...
conditionalPanel(condition = "input.num_windows >= 2",
                   checkboxGroupInput(inputId="winB_forecast",
                                      ...
                                      selected = c(9,10,11),
                                      inline=F, 
                                      label = "Window B Forecast Period"), # close checkbox
                   ...
                   renderUI({
                    lts_tweakb = adjustable_leadtimes2_reactive_b()
                     lts_tweakb %>%
                       sort() %>%
                       map(\(lt){
                         sliderInput(
                           inputId = paste0("sliderb_", lt),
                           label = paste("Adjust Leadtime B: ", lt),
                           min = 1, max = 30, value = 4  # Adjust min, max, and value as needed
                         )
                       }) #\close slider
                   }) # close renderUI
...
# for window 2
df_win2 <- eventReactive(input$winB_mon,{
  # browser()
  filter_forecast(df = df_ecmwf_zonal_all,
                  country= input$sel_country,
                  pub_mon = input$winB_mon,
                  valid_mon = input$winB_forecast
  )
})
df_insuv2 <- eventReactive(input$winA_mon,{
  gtm_selected<- ("Guatemala" %in% input$sel_country)
  if(!gtm_selected){
    ret <- NULL
  }
...
gather_slider_values_w2 <- reactive({
  ...
  set_names(lts_tweak,lts_tweak) %>% 
    map(\(lt){
      input[[paste0("sliderb_", lt)]]
    })
  
})

# you cant have reactive inside a static function like i do below..... i don't think therefore...
# gather_slider_values_static <-  function(pub_mo, valid_mo)

# therefore should really bring this one back into play
dynamic_rps_w1 <- eventReactive(input$winA_mon,{
...
dynamic_rps_w2 <- eventReactive(input$winB_mon,{
  l_rps <- gather_slider_values_w2()
  df_win_long <- df_win2() %>%
    select(-pub_date)
...
dynamic_rps_static <-  function(df){
  l_rps <- gather_slider_values()
  ...
  df_insuv_long <- df %>% 
   ...
   lts_in_df <- as.character(unique(df_insuv_long$lt))
...
#   dynamic_rps_static( df)
# })

dynamic_rps_insuv <- reactive({
 
  l_rps <- gather_slider_values()

  df_insuv_long <- df_insuv() %>% 
    select(-pub_date) 
...
apply_dynamic_rps_static <- function(df){
  df_thresh_wide <- dynamic_rps_static(df)
...
apply_dynamic_rps_rw1 <- reactive({
  df_thresh_wide <- dynamic_rps_w1()
  df_thresh_long<- df_thresh_wide %>%
    pivot_longer(
      cols= matches("\\b[0-6]\\b"),
      names_to ="lt",values_to = "q_ind"
...
apply_dynamic_rps_rw2 <- reactive({
  df_thresh_wide <- dynamic_rps_w2()
  df_thresh_long<- df_thresh_wide %>%
    pivot_longer(
      cols= matches("\\b[0-6]\\b"),
      names_to ="lt",values_to = "q_ind"
...
apply_dynamic_rps <-  reactive({
  # browser()
  apply_dynamic_rps_static(df = df_win1())
}
)

apply_dynamic_rps_ec_w2 <-  reactive({
  # browser()
  apply_dynamic_rps_static(df = df_win2())
}
)
apply_dynamic_rps_insuv <-  reactive({
  # browser()
  apply_dynamic_rps_static(df = df_insuv())
}
)
...
apply_dynamic_rps <- eventReactive(input$execute,{
  # browser()
  # df_thresh_wide <- dynamic_rps() # change back to this if not working
  df_thresh_wide <- dynamic_rps_static(df=df_win1())
  df_win <- df_win1()
  df_thresh_long<- df_thresh_wide %>%
    pivot_longer(
      cols= matches("\\b[0-6]\\b"),
...
render_gt({
  ...
  ecmwf_thresh <- apply_dynamic_rps_rw1()
...
```{r, echo=F}
  ...
  p_ret <- df_p %>% 
    ...
    geom_text(
      data= . %>%
        filter(lgl_flag),
      aes(x=pub_date, y= mm-50,label = format(pub_date,"%y")),
      size= 2
    )+
    theme(
...
```{r, echo=F}
# eventReactive(input$winB_mon,{
#   
#   # df_p <- apply_dynamic_rps_static(df = df_win1())$df
#   # l_ec_win1 <- apply_dynamic_rps_static(df = df_win1())
#   # df_ecmwf_thresh_hist <- l_ec_win1$df
#   # df_ecmwf_summarised_tbl <- l_ec_win1$rp_by_lt_indiv
#   # browser()
#   # 
#   l_ec_win2 <- apply_dynamic_rps_static(df = df_win2())
#   browser()
#   # df_ecmwf_thresh_hist2 <- l_ec_win2$df
#   # df_ecmwf_summarised_tbl2 <- l_ec_win2$rp_by_lt_indiv
#   
# })

renderTable({
  l_ec_win1 <- apply_dynamic_rps_rw1()
  browser()
  l_ec_win2 <- apply_dynamic_rps_rw2()
})
